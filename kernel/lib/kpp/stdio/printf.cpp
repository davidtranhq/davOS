...
int kpp::vprintf(const char* format, va_list args)
{
    size_t written = 0;
    while (*format != '\0') {
        ...
        // handle format specifiers
            ...
            if (format[0] == 'c') {
                ...
                format += 1;
                // standard mandates that variable arguments are promoted to at
                // least int, so we accept an int and convert it to a char
...
int kpp::printf(const char* __restrict format, ...)
{
    va_list args;
