...
Terminal::Terminal(limine_framebuffer* limineFramebuffer, Font font)
    : m_framebuffer {limineFramebuffer},
      m_font {font}
{}

template<typename CharacterType>
void Terminal::write(dav::StringView<CharacterType> string)
{
    for (auto character : string) {
        if (const bool firstVisibleCharacterWasChanged = pushCharacterToTextBuffer(character))
            repaintViewport();
        else
...
bool Terminal::pushCharacterToTextBuffer(CharacterType character)
{
    m_textBuffer.push(character);
    const aut o
    if (usesAutomaticScrolling() && m_textBuffer.linesBetween(m_firstVisibleCharacterInTextBuffer, m_textBuffer.endIndex) > maxVisibleLines())
        m_firstVisibleCharacterInTextBuffer = m_textBuffer.
}
